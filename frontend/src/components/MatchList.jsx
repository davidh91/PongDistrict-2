...
export default function MatchList() {
  ...
  if (loading) return <div className="text-center py-10 text-stone-400">Loading history...</div>

  return (
    <div>
      <h2 className="text-lg md:text-xl font-semibold mb-5 text-stone-800 flex items-center gap-2">
        <History className="w-4 h-4 text-stone-500" /> Recent Matches
      </h2>
      ...
      <div className="space-y-2">
        {matches.length === 0 ? (
          ...
          matches.map((match) => {
            ...
            return (
              <div key={match.id} className="bg-white/50 border border-stone-200 rounded-xl p-4 hover:bg-white/80 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-stone-400 font-mono mt-0.5">
                      {match.winner_elo_before} <span className="text-green-700">+{winnerGain}</span> → {match.winner_elo_after}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0 text-right">
                    <div className="font-medium text-stone-600 truncate">{match.loser.username}</div>
                    <div className="text-xs text-stone-400 font-mono mt-0.5">
                      {match.loser_elo_before} <span className="text-red-600">-{loserLoss}</span> → {match.loser_elo_after}
                    </div>
                  </div>
                </div>
              </div>
